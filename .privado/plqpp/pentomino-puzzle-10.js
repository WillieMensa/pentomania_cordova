/*	=============================================================================
	Pentomino Puzzle
	archivo: pentomino-puzzle-08.js
	24.09.2018
...
function initScreenVariable()
{
	...
	SCREEN_X = screenWidth;		// ancho y alto de pantallla adoptados
	SCREEN_Y = screenHeight;

	//	aqui tenemos detectados ancho y alto de pantalla
	//	definiremos ahora el sector a utilizar
	//	en una primera etapa lo haremos para pantalla vertical (forzada en celulares)
	if (SCREEN_Y > 1.33 * SCREEN_X)	// pantalla vertical > 4/3. muy alargada
	{
		...
		//	STAGE_OFFSET_X = Math.floor((screenWidth - 2* STAGE_X) * 0.03);
		//	if(STAGE_OFFSET_X < microCellSize) STAGE_OFFSET_X = microCellSize;
		STAGE_OFFSET_X = 2 * microCellSize;
		...
		//	STAGE_OFFSET_Y = Math.floor((screenHeight - 2 * STAGE_Y) * 0.03 );
		//	if(STAGE_OFFSET_Y < microCellSize) STAGE_OFFSET_Y = microCellSize;
		STAGE_OFFSET_Y = 2 * microCellSize;

	} else {
		// establecemos altura y luego ajustamos anchos
		STAGE_Y = ((screenHeight%2)?(screenHeight-1):screenHeight) - microCellSize*2;
		if(STAGE_Y > maxStageY) STAGE_Y = maxStageY;
		if(STAGE_Y < microStageY) STAGE_Y = microStageY;
		//	STAGE_OFFSET_Y = Math.floor((screenHeight - 2 * STAGE_Y) * 0.03 );
